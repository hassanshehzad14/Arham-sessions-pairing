...
router.get('/', async (req, res) => {
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                ...
                try {
                    ...
                    const sessionId = "ARHAM-MD~" + base64;

                    await sock.sendMessage(sock.user.id, { text: sessionId });

                    const successMsg = {
                        text:
                            `🚀 *ARHAM-MD Session Created!*\n\n` +
                            `▸ *Never share* your session ID\n` +
                            `▸ Join our WhatsApp Channel\n` +
                            `▸ Report bugs on Whatsapp\n\n` +
                            `_Powered by ARHAM-MD_\n\n` +
                            `🔗 *Useful Links:*\n` +
                            `▸ GitHub: https://github.com/arhammalik7860\n` +
                            `▸ https://whatsapp.com/channel/0029VbBOsFL96H4LmUFRHz3b`,
                        contextInfo: {
    ...
    try {
        ...
        removeFolder(tempDir);
        if (!res.headersSent) {
            res.status(500).json({ code: "Service Unavailable. Try again later." });
        }
    }
});
